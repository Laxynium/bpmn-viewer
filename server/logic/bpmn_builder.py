...
def to_a_graph(inMemoryGraph: InMemoryGraph) -> MyGraph:
    graph = MyGraph()
    graph.add_end_event(inMemoryGraph.end)
    for event in inMemoryGraph.events:
    ...
    for gateway in filter(lambda g: g[1] == "SPLIT", inMemoryGraph.and_gateways):
        ...
        graph.add_and_split_gateway(source, targets)
    ...
    for gateway in filter(lambda g: g[1] == "MERGE", inMemoryGraph.and_gateways):
        ...
        graph.add_and_merge_gateway(sources, target)
    ...
    for source, target in inMemoryGraph.edges.difference(inMemoryGraph.gatewaysEdges):
