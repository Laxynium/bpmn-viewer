...
class BpmnDiagramGraph(object):
    ...
    * plane_attributes - dictionary that contains BPMN plane element attributes.
    ...
    def get_flows_list_by_process_id(self, process_id):
        ...
        for flow in tmp_flows:
            if (
                consts.Consts.process in flow[2]
                and flow[2][consts.Consts.process] == process_id
            ):
                flows.append(flow)
        return flows
    ...
    def add_process_to_diagram(
        self,
        process_name="",
        process_is_closed=False,
        process_is_executable=False,
        process_type="None",
    ):
        """
        ...
        self.process_elements[process_id] = {
            consts.Consts.name: process_name,
    ...
    def add_task_to_diagram(self, process_id, task_name="", node_id=None):
        ...
        """
        return self.add_flow_node_to_diagram(
    ...
    def add_subprocess_to_diagram(
        ...
        subprocess_id, subprocess = self.add_flow_node_to_diagram(
            process_id, consts.Consts.subprocess, subprocess_name, node_id
        )
        self.diagram_graph.nodes[subprocess_id][consts.Consts.is_expanded] = (
    ...
    def add_start_event_to_diagram(
        self,
        process_id,
        start_event_name="",
        start_event_definition=None,
        ...
        """
        start_event_id, start_event = self.add_flow_node_to_diagram(
            process_id, consts.Consts.start_event, start_event_name, node_id
        )
        self.diagram_graph.nodes[start_event_id][consts.Consts.parallel_multiple] = (
            "true" if parallel_multiple else "false"
        )
        self.diagram_graph.nodes[start_event_id][consts.Consts.is_interrupting] = (
            "true" if is_interrupting else "false"
        )
        ...
        if start_event_definition == "message":
            event_def_list.append(
                BpmnDiagramGraph.add_event_definition_element(
                    "message", start_event_definitions
                )
            )
        elif start_event_definition == "timer":
            event_def_list.append(
                BpmnDiagramGraph.add_event_definition_element(
                    "timer", start_event_definitions
                )
            )
        elif start_event_definition == "conditional":
            event_def_list.append(
                BpmnDiagramGraph.add_event_definition_element(
                    "conditional", start_event_definitions
                )
            )
        elif start_event_definition == "signal":
            event_def_list.append(
                BpmnDiagramGraph.add_event_definition_element(
                    "signal", start_event_definitions
                )
            )
        elif start_event_definition == "escalation":
            event_def_list.append(
                BpmnDiagramGraph.add_event_definition_element(
                    "escalation", start_event_definitions
                )
            )

        self.diagram_graph.nodes[start_event_id][
            consts.Consts.event_definitions
        ] = event_def_list
        return start_event_id, start_event

    def add_end_event_to_diagram(
        self, process_id, end_event_name="", end_event_definition=None, node_id=None
    ):
        """
        ...
        end_event_id, end_event = self.add_flow_node_to_diagram(
            process_id, consts.Consts.end_event, end_event_name, node_id
        )
        ...
        if end_event_definition == "terminate":
            event_def_list.append(
                self.add_event_definition_element("terminate", end_event_definitions)
            )
        elif end_event_definition == "escalation":
            event_def_list.append(
                self.add_event_definition_element("escalation", end_event_definitions)
            )
        elif end_event_definition == "message":
            event_def_list.append(
                self.add_event_definition_element("message", end_event_definitions)
            )
        elif end_event_definition == "compensate":
            event_def_list.append(
                self.add_event_definition_element("compensate", end_event_definitions)
            )
        elif end_event_definition == "signal":
            event_def_list.append(
                self.add_event_definition_element("signal", end_event_definitions)
            )
        elif end_event_definition == "error":
            event_def_list.append(
                self.add_event_definition_element("error", end_event_definitions)
            )

        self.diagram_graph.nodes[end_event_id][
            consts.Consts.event_definitions
        ] = event_def_list
    ...
    def add_event_definition_element(event_type, event_definitions):
        ...
        event_def = {
            consts.Consts.id: event_def_id,
            consts.Consts.definition_type: event_definitions[event_type],
        }
        return event_def

    def add_gateway_to_diagram(
        self,
        process_id,
        gateway_type,
        gateway_name="",
        ...
        if not (
            gateway_direction in ("Unspecified", "Converging", "Diverging", "Mixed")
        ):
            raise bpmn_exception.BpmnPythonError(
                "Invalid value passed as gatewayDirection parameter. Value passed: "
                + gateway_direction
            )
        self.diagram_graph.nodes[gateway_id][
            consts.Consts.gateway_direction
        ] = gateway_direction
    ...
    def add_exclusive_gateway_to_diagram(
        ...
        """
        exclusive_gateway_id, exclusive_gateway = self.add_gateway_to_diagram(
            process_id,
    ...
    def add_inclusive_gateway_to_diagram(
        self,
        process_id,
        gateway_name="",
        gateway_direction="Unspecified",
        ...
        """
        inclusive_gateway_id, inclusive_gateway = self.add_gateway_to_diagram(
            process_id,
    ...
    def add_parallel_gateway_to_diagram(
        self, process_id, gateway_name="", gateway_direction="Unspecified", node_id=None
    ):
        """
        ...
        """
        parallel_gateway_id, parallel_gateway = self.add_gateway_to_diagram(
            process_id,
    ...
    def add_sequence_flow_to_diagram(
        self, process_id, source_ref_id, target_ref_id, sequence_flow_name=""
    ):
        """
        ...
        self.sequence_flows[sequence_flow_id] = {
            consts.Consts.name: sequence_flow_name,
